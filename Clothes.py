...
class Clothes(Product):
    ...
    def total_price(self, discount):
        if datetime.now().day == 8 and datetime.now().mounth == 3:  # если сегодня 8 марта
            ...
            return toFixed((self.price * 30) / 100,2)
        elif datetime.now().mounth == 8:  # если сегодня август, то скидка не верхнюю одежду 15%
            print(
                "Сегодня август, действует скидка на верхнюю одежду 15%, скидки не суммируются")
            return toFixed((self.price * 15) / 100,2)
        else:  # если ничего из этого то тогда дествуетобычная скидка покупателя
            return toFixed((self.price * discount) / 100,2)
